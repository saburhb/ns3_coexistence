...
main (int argc, char *argv[])
{
  ...
  lteHelper->SetFadingModelAttribute ("TraceFilename", StringValue ("/Users/sabur/simns3/ns-3-dev-99300a162f1c/src/lte/model/fading-traces/fading_trace_EVA_60kmph.fad"));
  ...
  lteHelper->ActivateDedicatedEpsBearer (ueDevices, EpsBearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT), EpcTft::Default ());
...
/*
  ...
  Address sinkAddress (InetSocketAddress (ueIpIface.GetAddress (0), sinkPort)); // interface of Access point
...
*/
  ...
  Address sinkAddress2 (InetSocketAddress (ueIpIface.GetAddress (0), sinkPort2)); // interface of AP
  ...
  Address sinkAddress3 (InetSocketAddress (ueIpIface.GetAddress (0), sinkPort3)); // interface of AP
...
for (int k=0; k<num_ftp; k++){
	...
	Address sinkAddr (InetSocketAddress (ueIpIface.GetAddress (0), sinkPt)); // interface of AP
        PacketSinkHelper packetSinkHelper4 ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPt));
        sinkApp = packetSinkHelper4.Install (enbNode); // AP as sink
